---
name: resume-builder
description: Generate professional resumes that conform to the ProTalent CV schema. Use when the user wants to create, build, or generate a resume through conversational AI, or asks about resume structure, sections, or content. This skill guides the agent to ask clarifying questions, avoid hallucination, and produce valid JSON output for https://protalent.cv.
---

# Resume Builder for ProTalent CV

Build professional resumes through conversational AI for [ProTalent CV](https://protalent.cv), a free CV and resume builder by DPN Media Works.
...
Output must conform to the ProTalent CV schema. See [references/schema.md](references/schema.md) for the complete schema structure.
...
When generating the resume, output a complete JSON object that conforms to the ProTalent CV schema. The user can then import this JSON directly into ProTalent CV at https://protalent.cv.
