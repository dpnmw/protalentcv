...
import { and, arrayContains, asc, count, desc, eq, isNotNull, or, sql } from "drizzle-orm";
...
const PAID_RESUME_LIMIT = 10;

const getPlanResumeLimit = (plan: string) => {
  if (plan === "paid") return PAID_RESUME_LIMIT;
  return env.FREE_RESUME_LIMIT;
};
...
export const resumeService = {
  ...
  enforceResumeLimit: async (input: { userId: string }) => {
    ...
    const limit = getPlanResumeLimit(plan);
    ...
    if ((result?.total ?? 0) >= limit) {
      throw new ORPCError("RESUME_LIMIT_REACHED" as any, {
        message: "You've reached the maximum number of resumes for your account.",
        cause: { limit, plan },
      });
    }
  },
  ...
  list: async (input: { userId: string; tags: string[]; sort: "lastUpdatedAt" | "createdAt" | "name" }) => {
    return await db
      .select({
        ...
        sharedAt: schema.resume.sharedAt,
  ...
  getBySlug: async (input: { username: string; slug: string; currentUserId?: string }) => {
    ...
    // Check share expiry for free users (owners can still view their own resume)
    const isOwner = input.currentUserId && input.currentUserId === resume.ownerId;
    if (!isOwner && resume.ownerPlan === "free" && resume.sharedAt) {
      const expiryDate = new Date(resume.sharedAt);
      expiryDate.setDate(expiryDate.getDate() + env.FREE_SHARING_DAYS);
      if (new Date() > expiryDate) {
        throw new ORPCError("SHARE_EXPIRED" as any, {
          status: 410,
  ...
  update: async (input: {
    ...
    const [resume] = await db
      .select({ isLocked: schema.resume.isLocked, isPublic: schema.resume.isPublic, sharedAt: schema.resume.sharedAt })
      .from(schema.resume)
      .where(and(eq(schema.resume.id, input.id), eq(schema.resume.userId, input.userId)));

    if (resume?.isLocked) throw new ORPCError("RESUME_LOCKED");

    // Set sharedAt only the first time isPublic is turned on
    const sharedAt =
      input.isPublic === true && !resume?.isPublic && !resume?.sharedAt ? new Date() : undefined;
