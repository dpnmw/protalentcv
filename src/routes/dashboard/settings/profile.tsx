...
import { ArrowSquareOutIcon, CheckIcon, ClockIcon, FileTextIcon, StarIcon, UserCircleIcon, WarningIcon } from "@phosphor-icons/react";
...
function RouteComponent() {
  ...
  const { session, planConfig } = Route.useRouteContext();
      ...
      <div className="max-w-xl rounded-lg border p-4">
        <div className="mb-3 flex items-center justify-between">
          <div className="flex items-center gap-x-2">
            <StarIcon weight="fill" className={isFree ? "text-muted-foreground" : "text-yellow-500"} />
            <h3 className="font-medium">
              {isFree ? <Trans>Free Account</Trans> : <Trans>Pro Account</Trans>}
            </h3>
          </div>

          {planConfig.enablePaidPlan && isFree && planConfig.upgradeUrl && (
            <a
              href={planConfig.upgradeUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-x-1.5 text-xs font-medium text-primary hover:underline"
            >
              <Trans>Upgrade</Trans>
        ...
        <div className="grid gap-2 text-sm text-muted-foreground">
          <div className="flex items-center gap-x-2">
            <FileTextIcon />
            <span>
              {isFree ? t`Resume limit: ${planConfig.freeResumeLimit} resumes` : t`Resume limit: unlimited`}
            </span>
          </div>
          ...
          <div className="flex items-center gap-x-2">
            <ClockIcon />
            <span>
              {isFree ? t`Public sharing: ${planConfig.freeSharingDays} days` : t`Public sharing: permanent`}
            </span>
          </div>
        </div>
      </div>
