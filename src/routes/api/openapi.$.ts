  ...
  if (request.method === "GET" && request.url.endsWith("/spec.json")) {
    const spec = await openAPIGenerator.generate(router, {
      info: {
        ...
        description: `${branding.appName} API`,
        license: { name: "MIT", url: `${branding.appUrl}/LICENSE` },
        contact: { name: branding.author.name, email: branding.author.email, url: branding.author.url },
      },
      servers: [{ url: `${env.APP_URL}/api/openapi` }],
      externalDocs: { url: branding.appUrl, description: `${branding.appName} Documentation` },
      commonSchemas: {
