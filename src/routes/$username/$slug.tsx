  ...
  return (
    <div className="flex min-h-screen items-center justify-center">
      <div className="text-center space-y-2">
        <h1 className="text-2xl font-semibold"><Trans>This resume link has expired.</Trans></h1>
        <p className="text-muted-foreground text-sm"><Trans>The owner's free share link is no longer active.</Trans></p>
      </div>
    </div>
  );
}
...
export const Route = createFileRoute("/$username/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: [{ title: loaderData ? `${loaderData.resume.name} - ProTalent CV` : "ProTalent CV" }],
  }),
  onError: (error) => {
    ...
    if (error instanceof ORPCError && (error as any).code === "SHARE_EXPIRED") {
      return;
    }
...
function RouteComponent() {
  ...
  const { data: resume, error } = useQuery(orpc.resume.getBySlug.queryOptions({ input: { username, slug } }));
