...
export function SharingSectionBuilder() {
  ...
  const shareExpiry = useMemo(() => {
    if (!resume.sharedAt) return null;
    const expiry = new Date(resume.sharedAt);
    expiry.setDate(expiry.getDate() + sharingDays);
    return expiry;
  }, [resume.sharedAt, sharingDays]);

  const isShareExpired = shareExpiry ? new Date() > shareExpiry : false;
      ...
      {resume.isPublic && (
        <div className="space-y-4 rounded-md border p-4">
          <div className="grid gap-2">
            ...
            {shareExpiry && (
              <p className={isShareExpired ? "text-destructive text-xs" : "text-muted-foreground text-xs"}>
                {isShareExpired ? (
                  <Trans>This share link has expired.</Trans>
                ) : (
                  <Trans>This link expires on {shareExpiry.toLocaleDateString()}.</Trans>
                )}
              </p>
            )}
          </div>
