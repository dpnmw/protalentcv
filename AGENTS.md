...
ProTalent CV is a single-package full-stack TypeScript app (not a monorepo) built on [Reactive Resume](https://github.com/AmruthPillai/Reactive-Resume) with [TanStack Start](https://tanstack.com/start/latest/docs/framework/react/overview) (React, Vite, Nitro). It serves both frontend and API on port 3000.
...
- `PRINTER_APP_URL` — must use the Docker bridge gateway IP (not localhost) so the Browserless container can reach the app on the host. Get the IP with: `sudo docker network inspect cvpro_default --format '{{range .IPAM.Config}}{{.Gateway}}{{end}}'`
